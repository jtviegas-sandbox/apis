...
public class Solution2Dto implements Function<org.aprestos.labs.apis.springboot2.model.entities.Solution, Solution> {

    @Override
    public Solution apply(org.aprestos.labs.apis.springboot2.model.entities.Solution solution) {
        Item[] items;
        if(null != solution.getItems() && !solution.getItems().isEmpty()){
            Item2Dto mapper = new Item2Dto();
            items = new Item[solution.getItems().size()];
            int i = 0;
            for( org.aprestos.labs.apis.springboot2.model.entities.Item item: solution.getItems() )
