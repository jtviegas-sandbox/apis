...
public class KnapsackStoreTests {
	...
	public void test_004_shouldGetTasksOnTheWhole() throws Exception {
		...
		mockMvc.perform(post("/knapsack/store/tasks").contentType("application/json")
				.content(jsonMapper.writeValueAsString(TaskWrapper.fromProblem(new Problem(RandomUtils.nextInt(1, 32),
						Arrays.asList(RandomUtils.nextInt(1, 32), RandomUtils.nextInt(1, 32),
								RandomUtils.nextInt(1, 32)),
						Arrays.asList(RandomUtils.nextDouble(1, 32), RandomUtils.nextDouble(1, 32),
								RandomUtils.nextDouble(1, 32)))))))
